...
def get_alpaca_client() -> Any:
    """Return a cached Alpaca TradingClient instance (paper trading)."""
    if not _check_keys():
        return None
...
def get_news_client() -> Any:
    """Return a cached Alpaca NewsClient instance."""
    if not _check_keys():
        return None
...
def fetch_account_activities(
    ...
    """Fetch account activities directly from the Trading API REST endpoint.

    ``TradingClient`` does not expose this method, so we call
    ``GET /v2/account/activities`` via raw HTTP.
    """
    if not _check_keys():
        return []
