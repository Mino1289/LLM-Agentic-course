...
The OpenAI-compatible embedding endpoint used by ``LLMProvider.embed()``
enforces a per-day quota (e.g. GitHub Models = 150 req/day on
``text-embedding-3-small``). Indexing 1,500+ chunks in a single bootstrap
run consumes the entire quota, so the next user query — which still has
to embed its own text — fails with ``429 RateLimitReached`` even though
the index itself is already populated.

This cache stores a SHA-256-keyed map of ``{text: vector}`` on disk so
subsequent identical queries are served without any network call.
"""
...
class EmbeddingCache:
    ...
    Entries are ``{ts: float, vector: list[float]}`` records keyed by
    SHA-256 of the query text. Entries older than ``ttl_seconds`` are
    pruned on load and on access. When ``max_entries`` is exceeded, the
    oldest entries are evicted (LRU-by-mtime).

    Thread-safety: ``get()`` is lock-free for reads (read-only dict), but
    ``put()`` / ``put_many()`` / ``clear()`` use an internal lock to make
...
def build_embedding_cache_from_env() -> EmbeddingCache:
    ...
    Env vars:
        EMBEDDING_CACHE_FILE         (default: data/embedding_query_cache.json)
        EMBEDDING_CACHE_TTL_SECONDS  (default: 604800 = 7 days)
        EMBEDDING_CACHE_MAX_ENTRIES  (default: 5000)
        EMBEDDING_CACHE_DISABLED     (default: 0; set to 1 to bypass the cache)
    """
    ...
    if os.getenv("EMBEDDING_CACHE_DISABLED", "0") == "1":
        # Return a cache pointed at /dev/null-ish path: a temp file in /tmp.
...
# Late import to avoid a circular dependency between paths.py / this module.
