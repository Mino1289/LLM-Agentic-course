...
class AsyncNodeThreadWrappingTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_tools_node_wraps_sync_execute_tool_in_to_thread(self):
        ...
        state = {
            "pending_tool_calls": [
                ToolCall(
                    ...
                    arguments='{"tickers": ["NVDA"], "start_date": "2024-01-01", "end_date": "2024-12-31"}',
                ),
            ],
        ...
        with unittest.mock.patch.object(
            tool_executor.asyncio, "to_thread", side_effect=spy
        ):
            try:
    ...
    async def test_multi_retrieve_node_wraps_rag_retrieve_in_to_thread(self):
        ...
        class _StubRAG:
            def retrieve(self, *a, **kw):
                return type(
                    "R", (), {"chunk_indices": [], "documents": [], "metadatas": []}
                )()
        ...
        with unittest.mock.patch.object(
            retrieval_node.asyncio, "to_thread", side_effect=spy
        ):
            try:
