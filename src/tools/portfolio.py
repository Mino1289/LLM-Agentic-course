...
def run_portfolio_info(args: PortfolioInfoArgs) -> dict[str, Any]:
    ...
    if positions:
        ...
        for pos in positions:
            ...
            lines.append(
                f"- **{sym}**: {qty:.4f} | Val.: {_fmt_usd(mkt_val)} | P&L: {_fmt_usd(upnl)} ({upnl_pct:+.2f}%)"
            )
    else:
    ...
    return {
        ...
        "account": {
            ...
            "unrealized_pl": round(
                float(account.equity) - float(account.last_equity or account.equity), 2
            ),
        },
        "positions": pos_list,
...
def run_portfolio_history(args: PortfolioHistoryArgs) -> dict[str, Any]:
    ...
    for i in range(len(history.timestamp)):
        ...
        lines.append(
            f"| {dt.strftime('%Y-%m-%d %H:%M')} | {_fmt_usd(eq)} | {_fmt_usd(pl) if pl else '$0'} | {plp:+.4f}% |"
        )
        data_points.append(
            {
                "timestamp": ts,
...
def run_account_activity(args: AccountActivityArgs) -> dict[str, Any]:
    ...
    for act in activities:
        ...
        lines.append(
            f"| {status} | {atype} | {symbol} | {qty_str} | {price_str} | {net_str} | {desc[:40]} |"
        )
    return {
        "text": "\n".join(lines),
