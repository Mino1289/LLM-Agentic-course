...
def run_place_trade(args: PlaceTradeArgs) -> dict[str, Any]:
    ...
    if ticker not in _TRACKED:
        return {"text": f"Ticker {ticker} non autorisé. Supportés: {', '.join(sorted(_TRACKED))}.", "error": "invalid_ticker"}
    ...
    if notional > _MAX_NOTIONAL_PER_ORDER:
        return {"text": f"## Ordre non exécuté — montant estimé trop élevé\nTicker: {ticker}\nQuantité: {qty}\nEstimation: {_fmt_usd(notional)} (max: {_fmt_usd(_MAX_NOTIONAL_PER_ORDER)})", "error": "notional_exceeded"}
    ...
    try:
        ...
        if order_type == "market":
            order_data = MarketOrderRequest(symbol=ticker, qty=qty, side=side_enum, time_in_force=time_in_force)
        elif order_type == "limit":
            if args.limit_price is None:
                return {"text": "Prix limite requis.", "error": "missing_limit_price"}
            order_data = LimitOrderRequest(symbol=ticker, limit_price=args.limit_price, qty=qty, side=side_enum, time_in_force=time_in_force)
        elif order_type == "stop":
            if args.stop_price is None:
                return {"text": "Prix stop requis.", "error": "missing_stop_price"}
            order_data = StopOrderRequest(symbol=ticker, stop_price=args.stop_price, qty=qty, side=side_enum, time_in_force=time_in_force)
        elif order_type == "stop_limit":
            if args.stop_price is None or args.limit_price is None:
                return {"text": "Prix stop et limite requis.", "error": "missing_stop_or_limit"}
            order_data = StopLimitOrderRequest(symbol=ticker, stop_price=args.stop_price, limit_price=args.limit_price, qty=qty, side=side_enum, time_in_force=time_in_force)
        else:
            return {"text": f"Type non supporté: {order_type}", "error": "unsupported_order_type"}
    ...
    return {
        ...
        "order": {"id": str(order.id), "symbol": ticker, "side": side, "qty": qty, "order_type": order_type, "status": str(order.status), "submitted_at": str(order.submitted_at)},
    }
...
def run_close_position(args: ClosePositionArgs) -> dict[str, Any]:
    ...
    try:
        if args.all:
            ...
            return {"text": f"## Positions liquidées\nToutes les positions ({count}) fermées.", "closed_count": count}
        ticker = args.ticker.upper().strip()
        if ticker not in _TRACKED:
            return {"text": f"Ticker {ticker} non autorisé. Supportés: {', '.join(sorted(_TRACKED))}.", "error": "invalid_ticker"}
