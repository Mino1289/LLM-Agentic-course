...
async def _balanced_rerank_indices(
    agent: Any, state: GraphState, candidates: list[int]
) -> list[int]:
    target_tickers = [
    ...
    groups = {
        ticker: [idx for idx in candidates if _candidate_ticker(agent, idx) == ticker]
...
async def rerank_node(agent: Any, state: GraphState) -> GraphState:
    ...
    stats.update(
        {
            "rerank_final_ticker_counts": _ticker_counts(final_meta),
