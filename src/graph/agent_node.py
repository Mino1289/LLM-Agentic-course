...
async def agent_node(agent: Any, state: GraphState) -> GraphState:
    ...
    async for stream_chunk in agent.rag.provider.ainvoke_with_tools_stream(
        ...
        if stream_chunk.usage:
            stats["llm_input_tokens"] = stream_chunk.usage.get(
                "prompt_tokens", stats.get("llm_input_tokens", 0)
            )
            stats["llm_output_tokens"] = stream_chunk.usage.get(
                "completion_tokens", stats.get("llm_output_tokens", 0)
            )
            stats["llm_total_tokens"] = stream_chunk.usage.get(
                "total_tokens", stats.get("llm_total_tokens", 0)
            )
        ...
        if stream_chunk.tool_call_delta:
            for tc_delta in stream_chunk.tool_call_delta:
                ...
                if tc_index is None:
                    tc_key = (
                        next(reversed(tool_calls_dict)) if tool_calls_dict else "tc_0"
                    )
                else:
                    tc_key = tc_index
                ...
                if tc_delta.get("thought_signature"):
                    tool_calls_dict[tc_key]["thought_signature"] = tc_delta[
                        "thought_signature"
                    ]
                if tc_delta.get("arguments"):
                    try:
                        ...
                        if isinstance(parsed, dict):
                            tool_calls_dict[tc_key]["arguments_parts"] = [
                                tc_delta["arguments"]
                            ]
                        else:
    ...
    if final_tool_calls:
        ...
        for tc in final_tool_calls:
            tool_events.append(
                {
                    "tool": tc.name,
                    "args_summary": summarize_tool_args(tc.name, tc.arguments),
