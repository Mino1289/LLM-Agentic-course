...
def _merge_rag_context(existing_chunks, existing_metadatas, new_chunks, new_metadatas):
    ...
    seen = set()
    pairs = [
        (c, existing_metadatas[i] if i < len(existing_metadatas) else {})
        for i, c in enumerate(existing_chunks)
    ]
    pairs.extend(
        (c, new_metadatas[i] if i < len(new_metadatas) else {})
    ...
    for key in sum_keys:
        try:
            merged[key] = int(merged.get(key, 0) or 0) + int(
                tool_stats.get(key, 0) or 0
            )
        except (TypeError, ValueError):
            continue
    merged["rerank_final_ticker_counts"] = _ticker_counts_from_metadatas(
        final_metadatas
    )
    ...
    *,
    ...
    if tool_name == "sec_filings_rag_tool":
        final_chunks, final_metadatas = _merge_rag_context(
            final_chunks,
            final_metadatas,
            result.get("final_chunks") or [],
            result.get("final_metadatas") or [],
        )
...
async def tools_node(agent: Any, state: GraphState) -> GraphState:
    ...
    for tool_call in pending:
        ...
        if signature in completed_signatures:
            ...
            stats["duplicate_tool_calls_skipped"] = (
                stats.get("duplicate_tool_calls_skipped", 0) + 1
            )
            continue
