...
from src.rag.metadata.extract import (
    extract_file_type_from_source,
    extract_year_from_source,
    extract_ticker_from_source,
)


def build_chunk_id(
    source: str, section: str, strategy: str, chunk_index: int, chunk: str
) -> str:
...
def build_metadata(
    source: str, section: str, chunk_index: int, chunk_strategy: str
) -> dict[str, Any]:
    return {
        "source": source,
        "section": section,
